...
var modeWork = function() {
	...
	stateApp.productivity.restNeeded = stateApp.productivity.restNeeded.add(
		moment.duration(
			stateApp.time.timeDiff * (stateApp.productivity.proportionRest / stateApp.productivity.proportionWork)
		)
	);
...
var productivity = function() {
	...
	stateApp.productivity.workAvailable = moment.duration(
		(stateApp.productivity.restLimit - stateApp.productivity.restNeeded) * 52 / 17
	);
...
var markPrayer = function(marked, markType) {
	...
	if (markType == 'currentSection') {
		$(selector)
			.parent()
			.children()
			.css('background-color', stateApp.settings.color.praySectionCurrent.backgroundColor);
