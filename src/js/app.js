...
let modeWork = function() {
  if (stateApp.mode.last !== stateApp.mode.current) {
    stateApp.productivity.workCurrent = moment.duration(0);
  }
  stateApp.productivity.restNeeded = stateApp.productivity.restNeeded.add(
    moment.duration(
      stateApp.time.timeDiff *
        (stateApp.productivity.proportionRest /
          stateApp.productivity.proportionWork)
    )
  );
  stateApp.productivity.workCurrent = stateApp.productivity.workCurrent.add(
    stateApp.time.timeDiff
  );
  stateApp.productivity.workTotal = stateApp.productivity.workTotal.add(
    stateApp.time.timeDiff
  );
};

let modeRest = function() {
  if (stateApp.mode.last !== stateApp.mode.current) {
    stateApp.productivity.restCurrent = moment.duration(0);
  }
  stateApp.productivity.restNeeded = stateApp.productivity.restNeeded.subtract(
    moment.duration(stateApp.time.timeDiff)
  );
  if (stateApp.productivity.restNeeded.asSeconds() < 0) {
    stateApp.productivity.restNeeded = moment.duration(0);
  }
  stateApp.productivity.restCurrent = stateApp.productivity.restCurrent.add(
    stateApp.time.timeDiff
  );
  stateApp.productivity.restTotal = stateApp.productivity.restTotal.add(
    stateApp.time.timeDiff
  );
};

let modeReset = function() {
  if (stateApp.mode.last !== stateApp.mode.current) {
...
let modeStop = function() {};

let modeTransition = function() {
...
let displayDuration = function(timeVar) {
  return `${timeVar.hours()}h ${timeVar.minutes()}m ${timeVar.seconds()}s`;
};

let productivity = function() {
  ...
  stateApp.productivity.workAvailable = moment.duration(
    (stateApp.productivity.restLimit - stateApp.productivity.restNeeded) *
      52 /
      17
  );
...
let checkLimit = function() {
  let audioLimit = document.getElementById('audio-limit');
  let limitPortion =
    stateApp.productivity.restNeeded.asSeconds() /
    stateApp.productivity.restLimit.asSeconds();
  if (limitPortion >= 1) {
...
let updatePrayerTime = function() {
  let today = new Date();
  let pray = prayTimes.getTimes(today, [
    stateApp.prayer.latitude,
    stateApp.prayer.longitude,
  ]);
  ...
  let nightLength =
    24 * 60 - stateApp.prayer.isha.diff(stateApp.prayer.fajr, 'minutes');
  let isha = stateApp.prayer.isha;
  let dateToday = stateApp.time.markCurrent.date();

  stateApp.prayer.oneThird = moment(isha)
    .add(nightLength * 1 / 3, 'minutes')
    .date(dateToday);
  stateApp.prayer.halfNight = moment(isha)
    .add(nightLength * 1 / 2, 'minutes')
    .date(dateToday);
  stateApp.prayer.twoThird = moment(isha)
    .add(nightLength * 2 / 3, 'minutes')
    .date(dateToday);

  stateApp.prayer.lastUpdate = moment();
};

let findPrayer = function(toFind) {
  if (toFind == 'nextPrayer') {
    let comparePrayer = [];
    let nextPrayerIndex = 0;
    for (let i = 0; i < stateApp.prayer.prayerList.length; i++) {
      comparePrayer.push(
        stateApp.time.markCurrent -
          stateApp.prayer[stateApp.prayer.prayerList[i]]
      );
    }
    ...
    for (let i = 0; i < stateApp.prayer.sectionList.length; i++) {
      compareSection.push(
        stateApp.time.markCurrent -
          stateApp.prayer[stateApp.prayer.sectionList[i]]
      );
    }
...
let markPrayer = function(marked, markType) {
  let selector = `#${marked.toLowerCase()}-time`;
  if (markType == 'currentSection') {
    $(selector)
      ...
      .css(
        'background-color',
        stateApp.settings.color.praySectionCurrent.backgroundColor
      );
    $(selector)
      .parent()
      .children()
      .css('color', stateApp.settings.color.praySectionCurrent.text);
    $(selector).css('font-weight', 'bold');
  } else if (markType == 'nextPrayer') {
    $(selector).css(
      'background-color',
      stateApp.settings.color.prayNext.backgroundColor
    );
    $(selector).css('color', stateApp.settings.color.prayNext.text);
  }
};

let timeToPrayer = function(prayerName) {
  let timeToNext = moment.duration(
    stateApp.prayer[prayerName] - stateApp.time.markCurrent
  );
  return timeToNext;
};
...
let prayer = function() {
  if (stateApp.prayer.lastUpdate === 0) {
    ...
    stateApp.time.markCurrent - stateApp.prayer.lastUpdate >=
    stateApp.settings.prayerUpdateRate
  ) {
    updatePrayerTime();
  }
...
let display = function() {
  ...
  $('#work-available').html(
    displayDuration(stateApp.productivity.workAvailable)
  );
